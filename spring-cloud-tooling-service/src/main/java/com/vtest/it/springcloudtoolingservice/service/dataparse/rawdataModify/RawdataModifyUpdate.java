...
import com.vtest.it.springcloudtoolingservice.service.dataparse.impl.VtMesConfigImpl;
...
public class RawdataModifyUpdate extends RawdataModifyParent {
    ...
    private ModiyFileCheck modiyFileCheck;
    private VtMesConfigImpl vtMesConfig;
    ...
    @Autowired
    public void setModiyFileCheck(ModiyFileCheck modiyFileCheck) {
        this.modiyFileCheck = modiyFileCheck;
    }

    @Autowired
    public void setVtMesConfig(VtMesConfigImpl vtMesConfig) {
        this.vtMesConfig = vtMesConfig;
    }

    public ArrayList<RawdataModifyBean> Transfer(File[] files, boolean adminFlag) {
        ...
        for (File csv : files) {
            ...
            try {
                if (!modiyFileCheck.check(csv)) {
                    throw new FileNotFoundException("there are error formart in your file,please check it again!");
                }
                if (adminFlag) {
                    if (!vtMesConfig.check(csv)) {
                        throw new FileNotFoundException("没配置LastStep，上传CP不在lastStep");
                    }
                }
